plot_cum_cancer_by_age <- function(
  ...
  # Updated required columns to check for mean_cum_cancer instead of bias
  required_cols <- c("time_t", "pct_cancer", "mean_cum_cancer", "pp_age_class")
  ...
  ggplot2::ggplot(
    results_plot_df,
    ggplot2::aes(x = pct_cancer, y = mean_cum_cancer) # Swapped y-axis variable
  ) +
    # Removed the yintercept = 0 geom_hline since we are no longer plotting bias around 0
    ggplot2::geom_point(
      ggplot2::aes(
        color = time_t_factor,
        shape = time_t_factor
    ...
    # Removed manual scale_y_continuous breaks to allow ggplot to adapt to large count numbers
