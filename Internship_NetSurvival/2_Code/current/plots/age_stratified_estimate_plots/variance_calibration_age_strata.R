plot_se_calibration_ratio_by_age <- function(
    results_df,
    title = "PPE Mean Estimated SE to Estimation Error SD Ratio Under Competing Risks by Age Class",
    x_lab = "Proportion of Cohort Experiencing Cancer Death vs Non-Cancer Death",
    y_lab = "Mean Estimated SE to Estimation Error SD Ratio",
    followup_lab = "Follow-up"
) {
  
  # Added pp_age_class to required columns
  required_cols <- c("time_t", "pct_cancer", "se_calibration_ratio", "pp_age_class")
  ...
  ggplot2::ggplot(
    ...
    ggplot2::scale_linetype_manual(
      values = c("No Mean Estimated SE to Estimation Error SD Ratio" = "dashed")
    ) +
