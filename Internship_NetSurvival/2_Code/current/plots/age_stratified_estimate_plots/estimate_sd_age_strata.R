plot_estimate_sd_by_age <- function(
    results_df,
    ...
    y_lab = "Empirical Dispersion (Estimate SD)",
    followup_lab = "Follow-up"
) {
  
  # Added pp_age_class to required columns
  required_cols <- c("time_t", "pct_cancer", "estimate_sd", "pp_age_class")
  ...
  ggplot2::ggplot(
    results_plot_df,
    ggplot2::aes(x = pct_cancer, y = estimate_sd)
  ) +
    ggplot2::geom_point(
      ggplot2::aes(
        color = time_t_factor,
    ...
    ggplot2::scale_linetype_manual(
      values = c("No Empirical Dispersion (Estimate SD)" = "dashed")
    ) +
