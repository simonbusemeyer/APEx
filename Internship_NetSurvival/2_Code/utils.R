...
# -----------------------------------------------------------------------------
# Courbe de survie théorique (individu i, temps t)
# -----------------------------------------------------------------------------
survtheo <- function(t, k, pho, alpha, beta_sex, beta_age, beta_X,
                     sex, ageStand, X) {
  ( 1 + ((pho*t)^k)/alpha )^(-alpha * exp(beta_sex * sex + beta_age * ageStand + beta_X * X))
}
...
plot_km_theo <- function(datasets, scenario, max_time) {
  ...
  # Courbe théorique : moyenne sur tous les individus et tous les datasets
  surv_theo_mean <- rowMeans(
    sapply(datasets, function(df) {
      # Pour chaque individu, calculer survtheo sur t_seq puis moyenner
